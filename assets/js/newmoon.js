...
function updateMoon() {
  ...
  if (fraction < 0.01) {
    ...
    // LOGIQUE CORRIGÉE basée sur le test visuel :
    // Pour une phase croissante à 97% d'illumination, on veut une PETITE ombre à gauche
    // Pour une phase croissante à 25% d'illumination, on veut une GRANDE ombre à droite
    
    if (isWaxing) {
      // Phase croissante (0 à 50%)
      if (fraction < 0.5) {
        // Première moitié croissante : ombre à droite qui diminue
        // À fraction=0, ellipseWidth=radius (ombre max à droite)  
        // À fraction=0.5, ellipseWidth=0 (ligne droite)
        ...
        // À fraction=0.5, ellipseWidth=0 (ligne droite)
        // À fraction=1, ellipseWidth=-radius (ombre max à gauche, mais négative)
        const ellipseWidth = radius * (2 * fraction - 1);
        // Pour les phases > 50%, l'ombre est à gauche, donc on inverse le sweep
        const sweepFlag = 0; // Pour ombre à gauche
